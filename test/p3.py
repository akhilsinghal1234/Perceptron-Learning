...
def arg_max(perceptron,x):
	...
	for i in range(len(perceptron)):
		...
		response[0].append(k)
		response[1].append(perceptron[i].pair)

	min_g = sys.maxsize
	for i in range(len(perceptron)):
		if response[0][i] < min_g:
			min_g = response[0][i]
			index = response[1][i][0]
	# print("min: ", min_g,"index: ",index)
	return index
...
def main():
	...
	for i in range(n):
		for j in range(i+1,n):
			trainset = []
			trainset = generateData(files,n-i-1,n-j-1)
			perceptron_i[k].train(trainset)
			perceptron_i[k].pairs(n-i,n-j)
			print(perceptron_i[k].pair)
			k += 1
